         ...
         <div class="card">
            <div class="card-body px-0">
                ...
                <div class="d-flex justify-content-end p-3">
                    <a href="{{ route('entrega.create') }}" class="btn btn-sm btn-primary ">
                    ...
                    <table id="user-list-table" class="table table-striped" role="grid" data-toggle="data-table">
                        ...
                        <tbody>
                            @foreach ($data as $item)
                                <tr>
                                    <td>{{ $item->codigo_conciliacion }}</td>
                                    <td>{{ $item->mes }}</td>
                                    <td>{{ $item->propietario->name }}</td>
                                    <td>{{ $item->tractoCamion->placa }}</td>
                                    <td>{{ $item->tramo->name }}</td>
                                    <td>{{ $item->product->name }}</td>
                                    <td>
                                        <div class="flex align-items-center list-user-action">
                                            <a href="{{ route('entrega.show', $item->id) }}" class="btn btn-sm btn-icon btn-info " data-toggle="tooltip"
                                            ...
                                            <a href="{{ route('entrega.edit', $item->id) }}" class="btn btn-sm btn-icon btn-warning"
                                            ...
                                            <form id="formDelete-{{ $item->id }}" action="{{ route('entrega.destroy', $item->id) }}" method="POST">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                        </div>
